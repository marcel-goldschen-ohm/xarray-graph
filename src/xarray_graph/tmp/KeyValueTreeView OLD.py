...
class KeyValueTreeView(TreeView):
    ...
    def customContextMenu(self, index: QModelIndex = QModelIndex()) -> QMenu:
        ...
        if item is model.rootItem():
            ...
            menu.addAction(QAction('Insert New', parent=menu, triggered=lambda checked, parent_item=item.parent, row=item.siblingIndex: self.insertNew(parent_item, row)))
